...
!/******************************************************************************
! *
! *  Authors: Juha Ruokolainen, Juhani Kataja
...
!>
!>  This solver is an experimental modification of the original magnetodynamics 
!>  solver with experimental magnetostriction capabilities added.
!>
!> \ingroup Solvers
!-------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
  ...
  LOGICAL :: Transient
!------------------------------------------------------------------------------
  LOGICAL :: Found, PiolaVersion, SecondOrder, LorentzConductivity, LagrangeGauge
...
!>  Allow magnetostrictive coupling with magnetostrictive linear elasticity solver
!> \ingroup Solvers
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
  ...
  REAL(KIND=dp), ALLOCATABLE :: LOAD(:,:), Acoef(:), Tcoef(:,:,:), &
                                ...
                                LamCond(:), Wbase(:), RotM(:,:,:), displacement(:,:)
...
!------------------------------------------------------------------------------
  ...
  CALL Info('WhitneyAVSolver','Solving the AV equations with edge elements, using EXPERIMENTAL magnetostriction solver',Level=5 )
  ...
  stab_c = GetCReal(GetSolverParams(), 'Linear System Stabilization Coefficient', Found)
...
300 CONTINUE
  ...
  DO t=1,active
     ...
     !ExternalHB = ListGetLogical( Material, 'External HB model', ExternalHB)
     !IF(ExternalHB) THEN
...
!------------------------------------------------------------------------------
    REAL(KIND=dp) :: Aloc(nd), JAC(nd,nd), mu, B_ip(3), Babs, dHdB(3,3), mu_ten(3,3), &
      H(3), strain_ten(3,3), LGrad(3,3), H_ip(3)
    REAL(KIND=dp) :: WBasis(nd,3), RotWBasis(nd,3), A, Acoefder(n), C(3,3), &
                     RotMLoc(3,3), RotM(3,3,n), velo(3), omega_velo(3,n), lorentz_velo(3,n)
    REAL(KIND=dp) :: Basis(n),dBasisdx(n,3),DetJ, L(3), G(3), M(3), FixJPot(nd), SaddleGauge(nd,nd)
...
!------------------------------------------------------------------------------
    ...
    DO t=1,IP % n
       ...
       DO i = 1,nd-np
         ...
         IF(Newton) FORCE(p) = FORCE(p) - SUM(RotWBasis(i,:)*H)*detJ*IP%s(t)
         ...
         DO j = 1,nd-np
           ...
           ELSE
             JAC(p,q) = JAC(p,q) + SUM(MATMUL(dHdB, RotWBasis(j,:)) * RotWBasis(i,:)) * &
               detJ*IP % s(t)
           END IF
    ...
    IF(LagrangeGauge) THEN
      IF( Newton ) THEN
        JAC(1:nd,1:nd) = JAC(1:nd,1:nd) + SaddleGauge(1:nd,1:nd)
      ELSE
...
!------------------------------------------------------------------------------
#ifdef _WIN32
  print *, 'Jfix and magnetostriction is not supported in windows, use virtual machine or CSC supercomputing environment'
  STOP
#else
...
!------------------------------------------------------------------------------
   ...
   REAL(KIND=dp) :: B(2,3), E(2,3), JatIP(2,3), VP_ip(2,3), JXBatIP(2,3), CC_J(2,3), B2, H_ip(2,3),  &
     LGrad(3,3), Strain_ten(3,3)
   ...
   DO i = 1, active
     ...
     DO j = 1,IP % n
       ...
       DO p=1,n
         ...
         IF ( ASSOCIATED(MFS).OR.ASSOCIATED(EL_MFS)) THEN
           IF (ExternalHB) THEN
             FORCE(p,k+1:k+3) = FORCE(p,k+1:k+3)+s*(H_ip(1,:)-REAL(MG_ip))*Basis(p)
           ELSE
