...
!------------------------------------------------------------------------------
SUBROUTINE MgsStressSolver_Init( Model,Solver,dt,Transient )
!------------------------------------------------------------------------------
    USE DefUtils
    IMPLICIT NONE
...
!------------------------------------------------------------------------------
    ...
    CALL ListAddString( SolverParams, &
      NextFreeKeyword('Exported Variable ', SolverParams), '-dofs 3 GNF' )

!------------------------------------------------------------------------------
  END SUBROUTINE MgsStressSolver_Init
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
   SUBROUTINE MgsStressSolver( Model,Solver,dt,Transient )
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
     ...
     CALL Info( 'MgsStressSolve', ' ', Level=4 )
     CALL Info( 'MgsStressSolve', '--------------------------------------------------',Level=4 )
     CALL Info( 'MgsStressSolve', 'Solving displacements from linear elasticity model',Level=4 )
     CALL Info( 'MgsStressSolve', '--------------------------------------------------',Level=4 )
...
!------------------------------------------------------------------------------
     ...
     IF( STDOFs < Mesh % MeshDim ) THEN
       CALL Fatal('MgsStressSolver','Number of Dofs smaller than dim: '&
...
!------------------------------------------------------------------------------
     IF ( .NOT. AllocationsDone .OR. Mesh % Changed) THEN
       ...
       IF ( istat /= 0 ) THEN
          CALL Fatal( 'MgsStressSolve', 'Memory allocation error.' )
       END IF
...
!------------------------------------------------------------------------------
     ...
     IF ( CalcStressAll ) THEN
       ...
       ELSE
         CALL Fatal('MgsStressSolver','Variable > Stress < does not exits!')
       END IF
       ...
       IF ( ASSOCIATED( Var ) ) THEN
         VonMises => Var % Values
       ELSE
         CALL Fatal('MgsStressSolver','Variable > vonMises < does not exits!')
       END IF
       ...
       IF(CalcPrincipalAll) THEN
         ...
         ELSE
           CALL Fatal('MgsStressSolver','Variable > Principal Stress < does not exits!')
         END IF
         ...
         IF ( ASSOCIATED( Var ) ) THEN
           Tresca => Var % Values
         ELSE
           CALL Fatal('MgsStressSolver','Variable > Tresca < does not exits!')
         END IF
         ...
         IF(CalcPrincipalAngle) THEN
           ...
           ELSE
             CALL Fatal('MgsStressSolver','Variable > Principal Angle < does not exits!')
           END IF
         END IF ! PrincipalAngle
       END IF !CalcPrincipalAll
     ...
     IF(CalculateStrains) THEN
       ...
       ELSE
         CALL Fatal('MgsStressSolver','Variable > Strain < does not exits!')
       END IF
       IF(CalcPrincipalAll) THEN
         ...
         ELSE
           CALL Fatal('MgsStressSolver','Variable > Principal Strain < does not exits!')
         END IF
       END IF
...
!------------------------------------------------------------------------------
     ...
     IF ( ModelLumping ) THEN
       IF(DIM /= 3) CALL Fatal('MgsStressSolve','Model Lumping implemented only for 3D')
       FixDisplacement = GetLogical( SolverParams, 'Fix Displacement', Found )
       IF(.NOT. Found) FixDisplacement = .TRUE.
       IF(FixDisplacement) THEN
         CALL Info( 'MgsStressSolve', 'Using six fixed displacement to compute the spring matrix' )
       ELSE
         CALL Info( 'MgsStressSolve', 'Using six pure forces and moments to compute the spring matrix' )
       END IF
       MinIter = 6
       MaxIter = 6
     ...
     DO iter=1,MaxIter
       ...
       IF( MaxIter > 1 ) THEN
         ...
         CALL Info( 'MgsStressSolve', Message,Level=4 )
       END IF
       CALL Info( 'MgsStressSolve', 'Starting assembly...',Level=5 )
!------------------------------------------------------------------------------
...
1000   CALL BulkAssembly()
       ...
       CALL Info( 'MgsStressSolve', 'Bulk assembly done', Level=5 )
...
3000   IF ( Transient .AND.(ConstantBulkMatrix .OR. &
       ...
       END IF
       CALL Info( 'MgsStressSolve', 'Set boundaries done', Level=5 )
     ...
     IF ( CalcStressAll .AND. .NOT. StabilityAnalysis ) THEN
       ...
       IF ( EigenAnalysis ) THEN
         ...
         DO i=1,nomodes
           ...
           CALL INfo('MgsStressSolver', Message )
             ...
             IF( i == 1 ) THEN
               ...
               IF( dofs > 1 ) THEN
                 DO k=1,dofs
                   ...
                   ELSE
                     CALL Fatal('MgsStressSolver','No variable associated: '//&
...
!------------------------------------------------------------------------------
     ...
     CALL Info('MgsStressSolver','All done',Level=4)
     CALL Info('MgsStressSolver','------------------------------------------',Level=4)
...
!------------------------------------------------------------------------------
       ...
       IF ( .NOT. Found )  THEN
         IF ( Transient .OR. EigenOrHarmonicAnalysis() ) &
            CALL Fatal( 'MgsStressSolve', 'No value for density found.' )
       END IF
           ...
           DO i=1,3
             ...
             IF( stat ) THEN
               ...
               IF( UnitNorm < EPSILON( UnitNorm ) ) THEN
                 CALL Fatal('MgsStressSolver','Given > Material Coordinate Unit Vector < too short!')
               END IF
       ...
       ! Collect magnetostriction model for current element
       !---------------------------------------------------
...
!------------------------------------------------------------------------------
     ...
     CALL Info('MgsStressSolver','------------------------------------------',Level=5)
     CALL Info('MgsStressSolver','Starting Stress Computation',Level=5)
     ...
     DO elem = 1,Solver % NumberOfActiveElements
      ...
      DO i=1,3
        DO j=i,3
          ...
          CALL Info('MgsStressSolver',Message,Level=5)
      ...
      CALL Info('MgsStressSolver','Finished Stress Computation',Level=5)
      CALL Info('MgsStressSolver','------------------------------------------',Level=5)
...
!------------------------------------------------------------------------------
       ...
       IF(.NOT. FoundBoundary) THEN
        CALL Fatal('MgsStressSolve','Model lumping boudary must be defined')
       END IF
   ...
   SUBROUTINE LumpedLoads( Permutation, Area, Center, Moments, Forces )
     ...
     ELSE IF(Permutation <= 6) THEN
       ...
       ELSE
         ...
         Forces(iy,1:n) = -c * (z(1:n) - Center(iz))
         Forces(iz,1:n) =  c * (y(1:n) - Center(iy))
         CALL Warn('MgsStressSolve','Moment matrix not diagonalazible!')
     ...
     ELSE
       DO t = 1, Mesh % NumberOfBoundaryElements
         ...
         IF ( .NOT. stat ) THEN
           ...
           IF ( .NOT. stat ) CALL Fatal( 'MgsStressSolve', &
...
!------------------------------------------------------------------------------
  ...
  END SUBROUTINE MgsStressSolver
!------------------------------------------------------------------------------
